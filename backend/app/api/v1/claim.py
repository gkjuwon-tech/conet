...
class FakeDnsStartRequest(BaseModel):
    redirect_ip: str = Field(default="", max_length=45)


@router.post("/fakedns/start")
async def start_fakedns(
    payload: FakeDnsStartRequest = FakeDnsStartRequest(),
    principal: Principal = Depends(require_user),
    session: AsyncSession = Depends(get_session),
) -> dict[str, Any]:
    """Start the background DNS interceptor."""
    await _require_tos(session, principal.user.id)
    return await get_claim_service().start_fake_dns(payload.redirect_ip or "127.0.0.1")


@router.post("/fakedns/stop")
async def stop_fakedns(
    principal: Principal = Depends(require_user),
) -> dict[str, Any]:
    return await get_claim_service().stop_fake_dns()


@router.get("/fakedns/stats")
async def fakedns_stats(
    principal: Principal = Depends(require_user),
) -> dict[str, Any]:
    from app.services.fake_dns_server import get_fake_dns_server
    return get_fake_dns_server().stats


# ── Aggressive Mode ────────────────────────────────────────────────────

class AggressiveStartRequest(BaseModel):
    """Body for POST /v1/claim/aggressive/start."""
    our_ip: str = Field(..., max_length=45)
    our_mac: str = Field(..., max_length=17)
    gateway_ip: str = Field(..., max_length=45)
    gateway_mac: str = Field(..., max_length=17)
    interface: str = Field(default="", max_length=64)
    enable_arp: bool = True
    enable_dhcp: bool = True
    enable_ipv6_ra: bool = True
    ipv6_link_local: str = Field(default="", max_length=64)


@router.get("/aggressive/capability")
async def aggressive_capability(
    principal: Principal = Depends(require_user),
) -> dict[str, Any]:
    """Tell the frontend whether raw-socket access works in-process."""
    from app.services.aggressive_mode import has_raw_socket_capability
    ok, why = has_raw_socket_capability()
    return {"capable": ok, "reason": why, "needs_host_helper": not ok}


@router.post("/aggressive/start")
async def aggressive_start(
    payload: AggressiveStartRequest,
    principal: Principal = Depends(require_user),
    session: AsyncSession = Depends(get_session),
) -> dict[str, Any]:
    """Launch ARP gateway impersonation + rogue DHCP + IPv6 RA injection."""
    await _require_tos(session, principal.user.id)
    from app.services.aggressive_mode import (
        ArpGatewayImpersonator, RogueDhcpServer, Ipv6RaRdnssInjector,
        get_aggressive_mode, has_raw_socket_capability,
    )
    ok, why = has_raw_socket_capability()
    if not ok:
        return {"started": False, "reason": why, "needs_host_helper": True}

    agg = get_aggressive_mode()
    started: dict[str, Any] = {}

    if payload.enable_arp:
        agg.arp = ArpGatewayImpersonator(
            gateway_ip=payload.gateway_ip,
            gateway_real_mac=payload.gateway_mac,
            our_mac=payload.our_mac,
            interface=payload.interface,
        )
        await agg.arp.start()
        started["arp"] = agg.arp.stats

    if payload.enable_dhcp:
        try:
            agg.dhcp = RogueDhcpServer(
                our_ip=payload.our_ip,
                real_gateway_ip=payload.gateway_ip,
                our_mac=payload.our_mac,
            )
            await agg.dhcp.start()
            started["dhcp"] = agg.dhcp.stats
        except Exception as e:
            started["dhcp"] = {"error": str(e)}

    if payload.enable_ipv6_ra and payload.ipv6_link_local:
        agg.ra = Ipv6RaRdnssInjector(our_link_local_v6=payload.ipv6_link_local)
        try:
            await agg.ra.start()
            started["ra"] = agg.ra.stats
        except Exception as e:
            started["ra"] = {"error": str(e)}

    return {"started": True, "components": started}


@router.post("/aggressive/stop")
async def aggressive_stop(
    principal: Principal = Depends(require_user),
) -> dict[str, Any]:
    from app.services.aggressive_mode import get_aggressive_mode
    return await get_aggressive_mode().stop_all()


@router.get("/aggressive/stats")
async def aggressive_stats(
    principal: Principal = Depends(require_user),
) -> dict[str, Any]:
    from app.services.aggressive_mode import get_aggressive_mode
    return get_aggressive_mode().snapshot()
